    ...
    while True:
        try:
            second_message = await client.ask(
                text="<b>Please Forward the Last Message/File from Channel DataBase. (Forward with Qoute)</b>\n\n<b>or Submit Post Link from Channel Database</b>",
                chat_id=message.from_user.id,
                filters=(filters.forwarded | (filters.text & ~filters.forwarded)),
                timeout=60,
            )
    ...
    while True:
        try:
            channel_message = await client.ask(
                text="<b>Please Forward Messages from Channel DataBase.(Forward with Qoute)</b>\n\n<b>or Submit Post Link from Channel Database</b>",
                chat_id=message.from_user.id,
                filters=(filters.forwarded | (filters.text & ~filters.forwarded)),
                timeout=60,
            )
